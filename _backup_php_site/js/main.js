/* ═══════════════════════════════════════════════════
   MN SPORTS — main.js  (multi-page)
   Shared: Preloader · Cursor · Navbar · Cart · Modal · Toast
...
/* ── CONFIG (dari database via window.MNS_DATA) ── */
const WA_NUMBER = (window.MNS_DATA && window.MNS_DATA.waNum) || '60123456789';
const CAT_LABEL = { football:'Football', casual:'Casual', formal:'Formal' };
const PRODUCTS  = (window.MNS_DATA && window.MNS_DATA.products) || [];
...
(function preloader() {
  ...
  function afterLoad() {
    ...
    startCounters();
    renderFeatured();
    const _pg = document.getElementById('productsGrid');
    if (_pg) renderProducts(_pg.dataset.defaultCat || 'all');
    initScrollReveal();
    handleUrlCat();
  }
...
(function setActiveNav() {
  ...
  document.querySelectorAll('.nav-links a, .mob-lnk').forEach(a => {
    const href = a.getAttribute('href');
    if (href && (href === page || href.startsWith(page + '?'))) a.classList.add('active');
  });
})();
...
function renderFeatured() {
  ...
  PRODUCTS.filter(p => p.featured).forEach((p, i) => {
    grid.appendChild(buildCard(p, i, 0.08));
  });
}
...
function renderProducts(filter) {
  ...
  if (!grid) return;
  grid.innerHTML = '';
  /* Category pages: only add matching products to DOM.
     products.php (filter='all'): add all so filter buttons can show/hide. */
  const list = filter === 'all'
    ? PRODUCTS
    : PRODUCTS.filter(p => p.category === filter);
  list.forEach((p, i) => {
    grid.appendChild(buildCard(p, i % 4, 0.07));
  });
}
...
/* URL param fallback: products.php?cat=football */
function handleUrlCat() {
  const cat = new URLSearchParams(location.search).get('cat');
  if (!cat) return;
  const btn = document.querySelector(`.f-btn[data-filter="${cat}"]`);
  if (btn) btn.click();
}
  ...
  inner.innerHTML = `
    ...
    <div class="pm-info">
      ...
      <div class="pm-price-row">
        ...
        <span class="pm-min">/ helai (min. 10 helai)</span>
      </div>
      ...
      <div class="pm-ctas">
        ...
        <a href="https://wa.me/${WA_NUMBER}?text=${waText}" target="_blank" rel="noopener" class="btn-wa-sm">
  ...
  form.querySelectorAll('.qin').forEach(inp => {
    inp.addEventListener('input', () => {
      ...
      if (qtyTotal) {
        ...
        qtyTotal.style.color = total < 10 ? '#e63012' : '#25d366';
      }
    });
  });
